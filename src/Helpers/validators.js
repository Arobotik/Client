...
export async function validateNewLoginData(state, loginChanged){
    let res = 0;
    if (loginChanged) {
        const response = await fetch('/api/validate/' + (state.login !== '' ? state.login : '-1'));
    ...
    if (!(state.password === '' && state.sessionId !== '')){
        if (!passValidate.test(state.password)){
            return 1;
        }
        if (state.password.length > 16 || state.password.length < 8){
            return 2;
        }
    }
    if (!loginValidate.test(state.login.toLowerCase()) || state.login.length > 25) {
        return 3;
    }
    return res;
}
...
export async function validateConnection(sessionId){
    if (sessionId === ''){
